...
function findVariantsForCell(tempData, y, x) {
	...
	for (var i = 0; i < 9; i++) {
		if (tempData[y][i] != 0 && variants.indexOf(tempData[y][i]) != -1)
			variants.splice(variants.indexOf(tempData[y][i]), 1)
		if (tempData[i][x] != 0 && variants.indexOf(tempData[i][x]) != -1)
...
function tryToSolve(tempData, y, x) { //Try to find correct digit for tempData[i][j]. If success return found digit else return 0
		...
		for (var i = 0; i < 9; i++)
			for (var j = 0; j < 9; j++)
				if (tempData[i][j] == 0) {
					...
					if (tempData[i][j] != 0) { //reduce numer of remaining cells
						remain--
						if (list)
							list.push({x : j, y : i})
					}
...
function canSolve(index) { //return true if this field can be solved else return false
	...
	tempData[Math.floor(index/9)][index%9] = 0
...
function removeOneCell() {
	...
	data[Math.floor(index/9)][index%9] = 0
}
